...
const renderers: Record<string, Renderer> = {
  ...
  'interactive-art-design': (ctx) => {
    ...
    const ShapeSVG = ({ shape }: { shape: string }) => {
      ...
      if (shape === 'star') {
        return (
          <svg width={size} height={size} viewBox="0 0 250 250" className="mx-auto" style={{ width: '100%', maxWidth: '400px', height: 'auto' }}>
        ...
        <div className="grid gap-4 md:grid-cols-2 print:grid print:grid-cols-1 print:gap-6 break-inside-avoid" style={{ gap: '16px', display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', pageBreakInside: 'avoid', breakInside: 'avoid' }}>
          {coloringPages.map((page, idx) => (
            <div key={idx} className="rounded-xl border border-purple-200 bg-white p-4 print:rounded-xl print:border print:border-purple-200 print:bg-white print:p-4 print:w-full break-inside-avoid" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact', backgroundColor: '#ffffff', borderColor: '#e9d5ff', padding: '16px', borderRadius: '12px', borderWidth: '1px', borderStyle: 'solid', marginBottom: '16px', pageBreakInside: 'avoid', breakInside: 'avoid', width: '100%' }}>
              ...
              <div className="mb-2 flex items-center justify-center print:mb-2 print:flex print:items-center print:justify-center" style={{ marginBottom: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px', backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #e9d5ff', minHeight: '400px', width: '100%' }}>
