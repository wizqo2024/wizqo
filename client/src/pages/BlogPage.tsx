...
![Herbs growing in jars](https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?auto=format&fit=crop&w=1600&q=80 "Growing herbs in jars on a desk")
...
![Tapping rhythms on a desk](https://images.unsplash.com/photo-1511379938547-c1f69419868d?auto=format&fit=crop&w=1600&q=80 "Practicing rhythms with everyday objects at home")
...
`,
    ...
    category: "Learning Tips",
    imageUrl: "https://images.unsplash.com/photo-1541961017774-2034504a1262?auto=format&fit=crop&w=1600&q=80",
...
Stop waiting for motivation to magically appear. Let Wizqo build your free 7-day hobby plan—personalized by AI, based on your life and energy.`,
    ...
    readTime: "5 min read",
    category: "Learning Tips",
    imageUrl: "https://images.unsplash.com/photo-1510936111840-65e151ad71bb?auto=format&fit=crop&w=1600&q=80",
    imageAlt: "Journal and pen — 7-day plan cover"
  },
  {
...
export function BlogPage({ initialSlug, onNavigate }: { initialSlug?: string; onNavigate?: (path: string) => void }) {
  ...
  const featurePost = useMemo(() => {
    ...
    const candidate = allPosts.find(p => !(p.id || '').includes('printable-coloring-pages')) || allPosts[0];
    return candidate;
  }, [allPosts]);
                ...
                if (selectedPost.id === 'relaxing-hobbies') {
                  ...
                  for (let idx = 0; idx < lines.length; idx++) {
                    ...
                    if (isMdImage(raw)) {
                      ...
                      if (!alt || !alt.trim()) alt = selectedPost.title;
          ...
          <div className="space-y-8">
            ...
            <article 
              className="bg-white rounded-2xl p-4 sm:p-6 lg:p-8 text-slate-900 cursor-pointer border border-slate-200 hover:border-purple-300 transition-all"
                  onClick={() => {
                    if (!safeFeaturePost) return;
                    setSelectedPost(safeFeaturePost);
                    navigateTo(`/blog/${safeFeaturePost.id}`);
                  }}
            >
              ...
              {(() => {
                const feature = safeFeaturePost || allPosts[0];
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4 mb-4 sm:mb-6 text-sm">
                <span className="flex items-center gap-2">
                  ...
                  {(safeFeaturePost || allPosts[0]).readTime}
                </span>
                <span className="flex items-center gap-2">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" />
                  </svg>
                  {(safeFeaturePost || allPosts[0]).date}
                </span>
                <span className="bg-white bg-opacity-20 px-2 py-1 rounded text-xs">
                  {(safeFeaturePost || allPosts[0]).category}
                </span>
                <div className="flex items-center gap-1 text-yellow-300">
                  ...
                  <span className="text-sm ml-1">({getPostRating((safeFeaturePost || allPosts[0]))})</span>
                </div>
              </div>
            </article>
              ...
              <div id="blog-results" className="grid md:grid-cols-2 gap-6">
                {(() => {
                  const cards = (isFilteringActive ? filteredPosts : allPosts)
                    .filter(p => (safeFeaturePost ? p.id !== safeFeaturePost.id : true));
                  if (cards.length === 0) {
                    return (
                      <div className="col-span-2 bg-white rounded-xl p-6 border border-slate-200 text-center text-slate-600">
