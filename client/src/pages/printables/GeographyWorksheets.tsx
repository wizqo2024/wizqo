...
export function GeoContinentsK2({ showAnswersForDoc }: { showAnswersForDoc: (docId: string, factory: () => ReactNode) => ReactNode }) {
    return (
                ...
                <svg viewBox="0 0 800 500" className="w-full h-auto drop-shadow-sm" role="img" aria-labelledby="continents-title">
                    ...
                    <g fill="white" stroke="#111827" strokeWidth="2" className="continent-shapes">
                        ...
                        <g>
                            ...
                            <text x="400" y="480" textAnchor="middle" fill="#111827" stroke="none" fontWeight="bold" fontSize="14">7</text>
                        </g>
                    </g>
                </svg>
            </div>
...
export function GeoCompassRose({ showAnswersForDoc }: { showAnswersForDoc: (docId: string, factory: () => ReactNode) => ReactNode }) {
    const { t } = useTranslation();

    // Helper to ensure getTrans is available or fallback to t
...
export function GeoLandforms({ effectiveSeed, variant, doc, showAnswersForDoc }: { effectiveSeed: string; variant: number; doc: string; showAnswersForDoc: (docId: string, factory: () => ReactNode) => ReactNode }) {
    const { t } = useTranslation();
    ...
    // Shuffle items
    const shuffledItems = React.useMemo(() => [...items].sort(() => rng() - 0.5), [rng, items]);
...
export function GeoLatLong({ showAnswersForDoc }: { showAnswersForDoc: (docId: string, factory: () => ReactNode) => ReactNode }) {
    const { t } = useTranslation();
