...
export function Add2DigitRegrouping({ showAnswersForDoc, seed, variant }: SpecificWorksheetProps) {
    ...
    function genPairs(count: number) {
        ...
        let guard = 0;
        while (out.length < count && guard < 10000) {
            const a = nextInt(15, 89);
            const b = nextInt(15, 89);
            // Regrouping ALWAYS needed condition: (a%10) + (b%10) >= 10
            if ((a % 10) + (b % 10) >= 10 && a + b <= 150) {
                 out.push([a, b]);
            }
            guard++;
        }
    ...
    const pairs = genPairs(12);

    return (
        <WorksheetSectionWrapper
            docId={docId}
            title={t(`worksheets.${docId}.title`, '2-Digit Addition (Regrouping)')}
            emoji="🔄"
            description={t(`worksheets.${docId}.description`, 'Add the numbers. Don\'t forget to carry the ten!')}
            problemCount={pairs.length}
        >
            <PremiumWorksheetBanner
                title="Regrouping Champ"
                subtitle="Carrying the Ten"
                icons={{
                    bg1: "🔄",
                    bg2: "➕",
                    float1: "1️⃣",
            ...
            <StrategySpotlight
                title="How to Regroup (Carry)"
                icon="🔄"
                steps={[
                    { label: "Step 1", text: "Add the Ones: 8 + 5 = 13" },
                    { label: "Step 2", text: "13 is 1 Ten and 3 Ones." },
                    { label: "Step 3", text: "Keep the 3 in the Ones place." },
                    { label: "Step 4", text: "Carry the 1 Ten to the Tens column!" }
                ]}
                color="indigo"
            />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-6">
                {pairs.map(([a, b], i) => (
                    <div key={i} className="flex flex-col items-center p-4 bg-indigo-50/30 rounded-xl border border-indigo-100 break-inside-avoid print:bg-white print:border-slate-300 relative">
                         {/* Carrying Box */}
                         <div className="absolute top-2 left-[38%] w-6 h-6 border border-indigo-200 rounded text-center text-xs text-indigo-400"></div>

                        <div className="flex flex-col items-end text-3xl font-mono font-bold text-slate-700 leading-tight w-24 mt-4">
                            <div>{a}</div>
                            <div className="border-b-4 border-slate-700 w-full flex justify-between">
...
export function Sub2DigitRegrouping({ showAnswersForDoc, seed, variant }: SpecificWorksheetProps) {
    ...
    function genPairs(count: number) {
        ...
        let guard = 0;
        while (out.length < count && guard < 10000) {
            const a = nextInt(20, 95);
            const b = nextInt(5, a - 1);
            
            // Regrouping ALWAYS needed condition: (a%10) < (b%10)
            // Example: 42 - 18 (2 < 8)
            if ((a % 10) < (b % 10)) {
                 out.push([a, b]);
            }
    ...
    return (
        <WorksheetSectionWrapper
            docId={docId}
            title={t(`worksheets.${docId}.title`, '2-Digit Subtraction (Regrouping)')}
            emoji="➖"
            description={t(`worksheets.${docId}.description`, 'Subtract the numbers. You may need to borrow from the tens!')}
            problemCount={pairs.length}
        >
            <PremiumWorksheetBanner
                title="Borrowing Hero"
                subtitle="Subtraction with Regrouping"
                icons={{
                    bg1: "➖",
                    bg2: "📉",
                    float1: "🔟",
            ...
            />
             ...
             <StrategySpotlight
                title="How to Borrow (Regroup)"
                icon="💡"
                steps={[
                    { label: "Problem", text: "42 - 18 (You can't do 2 - 8!)" },
                    { label: "Step 1", text: "Go to the Tens place (4). Borrow 1 Ten." },
                    { label: "Step 2", text: "Change the 4 to 3." },
                    { label: "Step 3", text: "Give the 10 to the Ones place. 2 becomes 12." },
                    { label: "Solve", text: "Now do 12 - 8 = 4!" }
                ]}
                color="rose"
            />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-6">
                {pairs.map(([a, b], i) => (
                    <div key={i} className="flex flex-col items-center p-4 bg-rose-50/30 rounded-xl border border-rose-100 break-inside-avoid print:bg-white print:border-slate-300 relative pt-8">
                         {/* Borrowing Boxes */}
                         <div className="absolute top-1 left-[30%] flex gap-2">
                            <div className="w-6 h-6 border border-rose-200 rounded text-center text-xs text-rose-400"></div>
                            <div className="w-6 h-6 border border-rose-200 rounded text-center text-xs text-rose-400"></div>
                         </div>
...
// ==========================================
export function FractionsHalvesThirdsFourths({ showAnswersForDoc, seed, variant }: SpecificWorksheetProps) {
    const { t } = useTranslation()
    const docId = 'fractions-halves-thirds-fourths'
    const rng = makeRng(`${seed}|v${variant}|doc=${docId}`)
    
    // Generate shapes with shaded parts
    const problems = Array.from({ length: 8 }, (_, i) => {
        const denom = pick([2, 3, 4], rng); 
        const num = Math.floor(rng() * denom) + 1; // 1 to denom (allow whole? maybe 1 to denom-1 for proper fractions usually)
        // Let's stick to proper fractions mostly, but having 2/2 or 3/3 is ok for instruction
        return { num, denom, shape: pick(['circle', 'rect'], rng) };
    });

    const renderShape = (p: { num: number, denom: number, shape: string }) => {
        const size = 100;
        const center = size / 2;
        const radius = 40;
        ...
        if (p.shape === 'circle') {
             ...
             for(let i=0; i<p.denom; i++) {
                 const startAngle = (i * 360) / p.denom;
                 const endAngle = ((i + 1) * 360) / p.denom;
                 
                 // Convert to radians (minus 90deg to start at top)
                 ...
                 paths.push(
                     <path 
                        key={i} 
                        d={d} 
                        fill={i < p.num ? '#4ade80' : 'white'} 
                        stroke="#000" 
                        strokeWidth="2"
                     />
                 );
             }
             return <svg viewBox={`0 0 ${size} ${size}`} className="w-32 h-32">{paths}</svg>;
        } else {
            // Rectangle
            // Horizontal strips
            const w = 80;
            const h = 80;
            const yStart = (size - h)/2;
            const xStart = (size - w)/2;
            const stripH = h / p.denom;
            
            return (
                 <svg viewBox={`0 0 ${size} ${size}`} className="w-32 h-32">
                     <rect x={xStart} y={yStart} width={w} height={h} fill="white" stroke="black" strokeWidth="2" />
                     {Array.from({length: p.denom}).map((_, i) => (
                         <rect 
                            key={i}
                            x={xStart}
                            y={yStart + i*stripH}
                            width={w}
                            height={stripH}
                            fill={i < p.num ? '#4ade80' : 'none'}
                            stroke="black"
                            strokeWidth="1"
                         />
                     ))}
                     <rect x={xStart} y={yStart} width={w} height={h} fill="none" stroke="black" strokeWidth="2" />
                 </svg>
            )
        }
    }
    ...
    return (
        <WorksheetSectionWrapper
            docId={docId}
            title={t(`worksheets.${docId}.title`, 'Fraction Action')}
            emoji="🍰"
            description={t(`worksheets.${docId}.description`, 'Color the Fraction. Or write the fraction for the shaded part.')}
            problemCount={problems.length}
        >
             <PremiumWorksheetBanner
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-8">
                {problems.map((p, i) => (
                    <div key={i} className="flex flex-col items-center p-4 bg-white border-2 border-green-100 rounded-xl break-inside-avoid">
                        {renderShape(p)}
                        <div className="mt-4 flex flex-col items-center gap-1">
                            <span className="text-sm font-bold text-slate-400">Write the fraction:</span>
                             <div className="flex flex-col items-center font-bold text-xl text-slate-800">
                                <div className="w-10 h-8 border-b-2 border-slate-800 bg-slate-50 flex items-center justify-center rounded-t"></div>
                                <div className="w-10 h-8 bg-slate-50 flex items-center justify-center rounded-b"></div>
                             </div>
                        </div>
                    </div>
                ))}
                    ...
                    <div className="grid grid-cols-4 gap-4 text-sm font-mono">
                         {problems.map((p, i) => (
                             <div key={i} className="text-center">
                                 #{i+1}: <strong>{p.num}/{p.denom}</strong>
                             </div>
                         ))}
                    </div>
                </div>
