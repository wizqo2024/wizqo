...
export function SplitChatInterface({ onGeneratePlan, onPlanGenerated, onNavigateBack }: SplitChatInterfaceProps) {
  ...
  const handleGoalSelect = async (goal: string) => {
    ...
    try {
      addAIMessage(`Perfect! I have everything I need. 🚀\n\nGive me a moment to create your personalized 7-day <span class=\"bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent font-semibold\">${selectedHobby}</span> learning plan...`);
  ...
  const handleOptionSelect = (value: string, messageId?: string) => {
    if (messageId) {
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="flex h-[calc(100vh-4rem)]">
                  ...
                  {message.options && (
                    <div className="mt-3 space-y-2">
                      {message.options.map((option) => (
                        <Button
                          key={option.value}
                          onClick={() => handleOptionSelect(option.value, message.id)}
                          disabled={disabledOptionMessageIds.has(message.id) || isGenerating}
                          aria-disabled={disabledOptionMessageIds.has(message.id) || isGenerating}
                          variant="outline"
        ...
        <div className="flex-1 flex flex-col relative">
          ...
          {/* Plan generating overlay until Day 1 exists */}
          {isGenerating && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-10">
              <div className="flex items-center gap-3">
                <div className="animate-spin rounded-full h-6 w-6 border-2 border-purple-500 border-t-transparent"></div>
                <span className="text-slate-700">Plan is generating…</span>
              </div>
            </div>
          )}
        </div>
