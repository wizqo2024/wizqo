...
// Lazy-load heavy embed for performance
...
export function SplitPlanInterface({ onGeneratePlan, onNavigateBack, initialPlanData }: SplitPlanInterfaceProps) {
  ...
  useEffect(() => {
      ...
      setSelectedDay(Math.max(1, Math.min(7, resume)));
    }
  } catch {}
}, []);
  ...
  const handleOptionSelect = async (value: string, label: string, messageId?: string) => {
    if (messageId) {
      setDisabledOptionMessageIds(prev => {
    ...
    if (currentStep === 'hobby') {
      ...
      addAIMessage(`Perfect! Creating your personalized <span class="bg-gradient-to-r from-purple-500 to-pink-500 bg-clip-text text-transparent font-semibold">${selectedHobby}</span> plan now... ✨`);
                  ...
                  {message.options && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {message.options.map((option) => (
                        <button
                          key={option.value}
                          onClick={() => handleOptionSelect(option.value, option.label, message.id)}
                          ...
                          disabled={isGenerating || disabledOptionMessageIds.has(message.id)}
                          aria-disabled={isGenerating || disabledOptionMessageIds.has(message.id)}
                        >
                          {option.label}
                        </button>
                      ))}
          ...
          {isGenerating && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center z-10">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-700">Plan is generating…</span>
              </div>
            </div>
          )}
        </div>
                          ...
                          <section className="bg-gradient-to-br from-red-50 to-pink-50 border border-red-200 rounded-2xl p-6 md:p-8">
                            ...
                            <div className="bg-white rounded-2xl p-6 shadow-sm">
                              {(() => {
                                ...
                                <React.Suspense fallback={<div className="text-center p-8">Loading video…</div>}>
                                  <YouTubeEmbed 
