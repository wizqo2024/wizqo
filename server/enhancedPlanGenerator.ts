...
export class EnhancedPlanGenerator {
	 ...
	 async generatePlan(request: PlanGenerationRequest): Promise<EnhancedPlan> {
		 ...
		 try {
			 ...
			 // Step 2: Map/validate structure
			 const validated = this.validateAndEnhancePlan(aiPlan, request);

			 // Step 3: Enhance with YouTube videos
			 const planWithVideos = await this.enhanceWithYouTubeVideos(validated, request.hobby);

			 // Step 4: Add affiliate products and resources
			 const enhancedPlan = await this.addAffiliateProducts(planWithVideos, request.hobby);

			 // Step 5: Save to Supabase if user is logged in
			 if (request.userId) {
	 ...
	 private async generateAIPlan(request: PlanGenerationRequest): Promise<any> {
...
- Make it achievable within ${request.timeAvailable} per day`;
	 ...
	 private async enhanceWithYouTubeVideos(plan: EnhancedPlan, hobby: string): Promise<EnhancedPlan> {
		 ...
		 const enhancedDays = await Promise.all(
			 plan.days.map(async (day, index) => {
				 try {
					 // Preserve AI-provided video if valid
					 if (day.youtubeVideoId && typeof day.youtubeVideoId === 'string' && day.youtubeVideoId.length > 5 && !day.youtubeVideoId.startsWith('test-')) {
						 const details = await this.getVideoDetails(day.youtubeVideoId);
						 return { ...day, videoTitle: day.videoTitle || details?.title || `Day ${day.day} Tutorial` };
					 }
					 ...
					 if (videoId) {
						 ...
						 return { ...day, youtubeVideoId: videoId, videoTitle: videoDetails?.title || `Day ${day.day} Tutorial` };
					 }
				 } catch (error) {
					 console.log(`⚠️ Could not get video for day ${day.day}:`, error);
				 }
				 // Fallback to generic video
				 return { ...day, youtubeVideoId: this.getFallbackVideoId(hobby, index), videoTitle: `${hobby} Day ${day.day} Tutorial` };
			 })
		 );

		 return { ...plan, days: enhancedDays };
	 }
	 ...
	 private async getTargetedVideoForDay(hobby: string, day: number, dayTitle: string): Promise<string | null> {
		 ...
		 try {
			 ...
			 const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(searchQuery)}&type=video&maxResults=1&key=${this.youtubeApiKey}`);
	 ...
	 private async getVideoDetails(videoId: string): Promise<{ title: string } | null> {
		 ...
		 try {
			 ...
			 const data = await response.json();
			 const video = data.items?.[0];
			 if (video) return { title: video.snippet.title };
		 } catch (error) {
			 console.error('❌ YouTube API Error:', error);
		 }
		 return null;
	 ...
	 private getAffiliateProducts(hobby: string, day: number): { title: string; link: string; price: string }[] {
		 ...
		 return [hobbyProductList[(day - 1) % hobbyProductList.length]];
	 }
	 ...
	 private validateAndEnhancePlan(plan: any, request: PlanGenerationRequest): EnhancedPlan {
		 const validated: EnhancedPlan = {
			 hobby: plan.hobby || request.hobby,
			 title: plan.title || `Master ${request.hobby} in 7 Days`,
			 overview: plan.overview || plan.description || `A comprehensive 7-day journey to learn ${request.hobby}`,
		 ...
		 for (let i = 0; i < 7; i++) {
			 const d = plan.days?.[i] || {};
			 const mainTask = d.mainTask || d.goal || d.objective || d.content || `Learn ${request.hobby} fundamentals`;
			 const explanation = d.explanation || d.description || d.details || d.reasoning || `Day ${i + 1} of your ${request.hobby} learning journey`;
			 const howTo = d.howTo || d.steps || d.stepByStep || d.instructions;
			 const checklist = d.checklist || d.actionItems || d.todos;
			 const tips = d.tips || d.proTips || d.suggestions;
			 const mistakes = d.mistakesToAvoid || d.commonMistakes || d.avoidMistakes || d.pitfalls;
			 const aiVideoId = d.youtubeVideoId || d.videoId || d.youtube;

			 validated.days.push({
				 day: i + 1,
				 title: (typeof d.title === 'string' && d.title.trim().length > 0) ? d.title : `Day ${i + 1}`,
				 mainTask,
				 explanation,
				 howTo: Array.isArray(howTo) && howTo.length > 0 ? howTo : [`Step ${i + 1}`],
				 checklist: Array.isArray(checklist) && checklist.length > 0 ? checklist : [`Complete day ${i + 1} tasks`],
				 tips: Array.isArray(tips) && tips.length > 0 ? tips : [`Tip for day ${i + 1}`],
				 mistakesToAvoid: Array.isArray(mistakes) && mistakes.length > 0 ? mistakes : [`Avoid rushing on day ${i + 1}`],
				 estimatedTime: d.estimatedTime || request.timeAvailable,
				 skillLevel: d.skillLevel || request.experience,
				 youtubeVideoId: (typeof aiVideoId === 'string' && aiVideoId.length > 5 && !aiVideoId.startsWith('test-')) ? aiVideoId : undefined,
				 freeResources: [],
				 affiliateProducts: []
			 });
		 }
