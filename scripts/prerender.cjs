...
function cloneForRoute(baseHtml, route, allPosts = [], allRoutes = []) {
  ...
  const richContent = directRichContent || route.richContent;
  ...
  // 2. Otherwise fall back to hardcoded overrides (legacy)
    ...
    if (slug && richContent) {
      ...
      // EMERGENCY OVERRIDE: Hardcode content if missing from JSON
      if (slug === 'sub-2digit-100') {
        const richContentOverride = `<article style="max-width: 1200px; margin: 0 auto; color: #1e293b; line-height: 1.6;">
  ...
  <section style="margin-top: 2.5rem; border-top: 1px solid #e2e8f0; padding-top: 2rem;">
    ...
    <p style="color: #475569; line-height: 1.6; margin-bottom: 1rem;">At Wizqo, we believe that high-quality education should be accessible to everyone. Our resources are designed by pedagogical experts to ensure they meet the highest standards of clarity. By focusing on print-based learning, we help students foster a deeper connection with the material through physical problem-solving.</p>
        ...
        html = html.replace(/<main[^>]*id="seo-fallback"[^>]*>[\s\S]*?<\/main>/, `<main id="seo-fallback">${richContentOverride}</main>`);
        console.log('  ✓ EMERGENCY INJECTION: sub-2digit-100');
      } else if (slug === 'addition-subtraction-within-10' || slug === 'addition-subtraction-0-10') { // Check both potential slugs
        const richContentOverride = `<article style="max-width: 1200px; margin: 0 auto; color: #1e293b; line-height: 1.6;">
  ...
  <section style="margin-top: 2.5rem; border-top: 1px solid #e2e8f0; padding-top: 2rem;">
    ...
    <p style="color: #475569; line-height: 1.6;">Wizqo produces educator-vetted math tools that are high-fidelity for student growth. Our primary series follows rigorous developmental standards for early arithmetic excellence. Download your first step today!</p>
        ...
        html = html.replace(/<main[^>]*id="seo-fallback"[^>]*>[\s\S]*?<\/main>/, `<main id="seo-fallback">${richContentOverride}</main>`);
        // Also perform title override if we are here
        html = setTitle(html, "Addition and Subtraction within 10 - Math Foundations | Wizqo");
        console.log('  ✓ EMERGENCY INJECTION: addition-subtraction-within-10');
      } else {
