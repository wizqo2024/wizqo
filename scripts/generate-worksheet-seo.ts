...
function inferCategory(docId: string): string[] {
  ...
  if (docId.includes('reading') || docId.includes('comprehension') || docId.includes('literacy')) {
    categories.push('reading')
  }
  if (docId.includes('writing') || docId.includes('handwriting') || docId.includes('cursive') || docId.includes('trace')) {
    categories.push('writing')
  }
  if (docId.includes('phonics') || docId.includes('cvc') || docId.includes('rhyme')) {
    categories.push('phonics')
  }
  if (docId.includes('alphabet') || docId.includes('letter')) {
    categories.push('alphabet')
  }
  if (docId.includes('fine-motor') || docId.includes('cut-and-paste') || docId.includes('coloring')) {
    categories.push('fine-motor')
  }

  return categories.length > 0 ? categories : ['general-education']
}
...
function generateMetaDescription(name: string, grade: string[], category: string[]): string {
  ...
  const isMath = category.some(c => ['multiplication', 'division', 'fractions', 'addition-subtraction', 'counting', 'number-sense', 'math', 'geometry', 'measurement'].includes(c))
  const practiceType = isMath ? 'math practice' : 'educational practice'
  const skillsType = isMath ? 'math skills' : 'essential skills'

  return `Download free printable ${name.toLowerCase()} worksheet with answer key. ${gradeText}${practiceType}. Build ${skillsType}. Instant PDF download.`
}
...
function generateKeywords(name: string, category: string[], grade: string[]): string {
  ...
  const isMath = category.some(c => ['multiplication', 'division', 'fractions', 'addition-subtraction', 'counting', 'number-sense', 'math', 'geometry', 'measurement'].includes(c))
  const cat = category.map(c => `${c} worksheet`)
  const grd = grade.map(g => isMath ? `${g} math worksheet` : `${g} reading worksheet`)
...
function generateIntro(name: string, grade: string[], category: string[]): string {
  ...
  const isMath = category.some(c => ['multiplication', 'division', 'fractions', 'addition-subtraction', 'counting', 'number-sense', 'math', 'geometry', 'measurement'].includes(c))
  const subjectSkills = isMath ? 'math skills' : 'essential literacy and learning skills'
